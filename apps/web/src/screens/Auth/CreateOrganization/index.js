...
const CreateOrganization = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    let role = 'admin';
    let token = jwt_token;
    const headers = { Authorization: `Bearer ${authState.user.jwt_token}` };
    ...
    // Moved to up on stacktrace: Save user data to global state
    // let user = { id: user_id, username, jwt_token, email };
    // await LogIn(user);
  ...
  return (
    <React.Fragment>
      ...
      <Wrapper>
        ...
        <Title>Thank You for register, your account is ready to use</Title>
        <Spin
        ...
        >
          <AuthCard>
            {isInviteFlow === 'true' ? (
              <div>
                <CardText>
                  Click below to navigate to the app your were invited to
                </CardText>
                <TextWrapper>
